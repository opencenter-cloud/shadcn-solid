...
export const RadioGroupItemControl = <T extends ValidComponent = "div">(
  ...
  return (
    <RadioGroupPrimitive.ItemControl
      ...
      class={cx(
        "border-input dark:bg-input/30 shadow-xs flex size-4 items-center justify-center rounded-full border outline-none transition-[color,box-shadow]",
        "disabled:(cursor-not-allowed opacity-50)",
        "data-[invalid]:(ring-destructive/20 dark:ring-destructive/40 border-destructive)",
        "peer-focus-visible/radio_group:(ring-ring/50 border-ring ring-[3px])",
        props.class,
      )}
      {...rest}
    />
...
export const RadioGroupLabel = <T extends ValidComponent = "span">(
  ...
  return (
    <RadioGroupPrimitive.Label
      ...
      class={cx(
        "select-none text-sm font-medium data-[invalid]:text-destructive",
        props.class,
      )}
      {...rest}
    />
