...
export const Slider = <T extends ValidComponent = "div">(
  ...
  return (
    <SliderPrimitive
      data-slot="slider"
      class={cx(
        "relative flex w-full touch-none select-none flex-col items-center gap-2 data-[disabled]:opacity-50",
        "data-[orientation=vertical]:(h-full min-h-44 w-auto)",
        props.class,
      )}
      {...rest}
    />
...
export const SliderTrack = <T extends ValidComponent = "div">(
  ...
  return (
    <SliderPrimitive.Track
      ...
      class={cx(
        "bg-muted relative rounded-full",
        "data-[orientation=horizontal]:(h-1.5 w-full)",
        "data-[orientation=vertical]:(h-[inherit] min-h-[inherit] w-1.5)",
        props.class,
      )}
      {...rest}
    />
...
export const SliderThumb = <T extends ValidComponent = "span">(
  ...
  return (
    <SliderPrimitive.Thumb
      ...
      class={cx(
        "border-primary bg-background ring-ring/50 size-4 rounded-full border shadow-sm transition-[color,box-shadow] hover:ring-4 data-[orientation=horizontal]:-top-1 data-[orientation=vertical]:-left-1",
        "focus-visible:(outline-hidden ring-4)",
